...
const loginUser = async (entitiesmanager, credentials, authRepo) => {
    ...
    if (user) {
        ...
        if (isMatch) {
            const token = await (0, TokenGenerator_1.genrateToken)({ id: user.user_id, role: "ADMIN" });
            await authRepo.saveRefreshToken(entitiesmanager, { id: user.user_id, refreshToken: token.refreshToken });
