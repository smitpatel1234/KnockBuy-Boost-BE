...
export const OrderRepo: OrderRepoPort = {
  ...
  GetAllOrdersPage: async (em: EntityManager, data: pageParams): Promise<PaginationResponse<OrderAllType>> => {
    const qb = em.getRepository(Order).createQueryBuilder("order").leftJoin("order.user", "user")
      .select([
  ...
  placeOrder: async (em: EntityManager, data: PlaceOrder) => {
    ...
    if (cartItems.length === 0)  throw new ApplicationError( ApplicationErrorType.NOT_FOUND,"Cart is empty");
    let subtotal = 0;
    ...
    for (const cartItem of cartItems) 
      if (cartItem.item.stock < cartItem.quantity)
        throw new ApplicationError( ApplicationErrorType.BAD_REQUEST, `Insufficient stock for item: ${cartItem.item.item_name}`);
    ...
    const orderItems: OrderItems[] = cartItems.map((cartItem) => em.create(OrderItems,
  ...
  UpdateOrder: async (em: EntityManager, data: UpdateOrderModel): Promise<boolean> => {
    ...
    if (data.address_id) existing.address = { address_id: data.address_id } as unknown as Address;
